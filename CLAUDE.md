...
AI Dev Team Builder is a bash-based orchestration tool that creates an AI-powered development team using tmux and Claude CLI. It dynamically creates tmux panes with different Claude instances, each assigned a specific role (manager, developer, tech lead, etc.) to collaborate on software development projects.
...
./setup-claude-workflow.sh <worktree-name>
```
...
- `ROLE_DIR`: Role files directory (default: `./claude-workflow-roles`)
...
1. Ensure bash syntax is valid: `bash -n setup-claude-workflow.sh`
...
3. **Tmux Pane Management**:
   ...
   - Panes 2-N: Dynamically created based on role files
...
- **No Build System**: Pure bash script requiring only system dependencies (tmux, git, claude CLI)

## Development Workflow

1. **Adding New Roles**: Create a new `.md` file in `claude-workflow-roles/`
2. **Modifying Script Behavior**: Edit `setup-claude-workflow.sh` directly
...
- The tool assumes Claude CLI is installed globally via npm